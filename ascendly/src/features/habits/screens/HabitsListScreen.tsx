...
const HabitsListScreen = () => {
  ...
  const habits = useMemo(() => {
    ...
    return allHabits
      .filter(h => {
        ...
        const isInDateRange = (!habitStart || selectedDate >= habitStart) && (!habitEnd || selectedDate <= habitEnd);
  ...
  const activeHabitsCount = useMemo(() => habits.filter(h => h.status === 'active').length, [habits]);
  ...
  const renderHabitItem = useCallback(
    ({item}: {item: Habit}) => (
      <HabitCard item={item} onPress={handleHabitPress} onActionPress={handleActionPress} selectedDate={selectedDate} />
    ),
    [handleHabitPress, handleActionPress, selectedDate]
  );

  const renderSectionHeader = useCallback(
    ({section: {title, icon}}: any) => (
      <View style={styles.sectionHeader}>
        <Icon name={icon} size={18} color={colors.textSecondary} style={styles.sectionHeaderIcon} />
        <AppText style={[styles.sectionHeaderText, {color: colors.textSecondary}]}>{title.toUpperCase()}</AppText>
      </View>
    ),
    [colors, styles]
  );
    ...
    <SafeAreaView style={[styles.container, styles.themedContainer]} edges={['top']}>
      ...
      <View style={styles.content}>
        ...
        <SectionList
          ...
          ListHeaderComponent={
            <HabitListHeader habits={habits} selectedDate={selectedDate} onDateSelect={setSelectedDate} />
          }
      ...
      {showDatePicker && (
        <DateTimePicker
          value={
            new Date(
              selectedDate.split('-').map(Number)[0],
              selectedDate.split('-').map(Number)[1] - 1,
              selectedDate.split('-').map(Number)[2]
            )
          }
          mode="date"
