            ...
            <>
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                        ...
                        <TouchableOpacity
                          ...
                          <View
                            style={[
                              ...
                              {backgroundColor: item.color + (isSelected ? '20' : '10')},
                            ]}>
                            <Icon name={item.icon} size={24} color={item.color} />
                          </View>
                          <AppText
                            style={[styles.categoryName, {color: isSelected ? item.color : colors.textSecondary}]}>
                            {item.name}
                          </AppText>
                        </TouchableOpacity>
                      );
                    ...
                    {(['low', 'medium', 'high'] as HabitPriority[]).map(p => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          <AppText style={[styles.priorityText, {color: isSelected ? pColor : colors.textSecondary}]}>
                            {p.toUpperCase()}
                          </AppText>
                        </TouchableOpacity>
                      );
                    ...
                    <View style={styles.gap12}>
                      {values.completions
                        ...
                        .map((c: HabitCompletion, index: number) => (
                          <View key={`${c.date}-${index}`} style={styles.noteCard}>
                            <AppText style={styles.noteDateText}>{c.date}</AppText>
                            <TextInput
                  ...
                  <>
                        ...
                        {DAYS.map(day => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              <AppText
                                style={[styles.dayButtonText, {color: isSelected ? '#FFFFFF' : colors.textSecondary}]}>
                                {day}
                              </AppText>
                            </TouchableOpacity>
                          );
                    ...
                    <View style={styles.section}>
                      ...
                      <View
                        style={[
                          styles.inputContainer,
                          focusedField === 'targetPerWeek' && styles.inputContainerFocused,
                          values.daysTarget.length > 0 && {opacity: 0.6},
                        ]}>
                        <TextInput
                  ...
                  <>
                    ...
                    <View style={styles.section}>
                      ...
                      <View
                        style={[
                          styles.inputContainer,
                          focusedField === 'targetPerMonth' && styles.inputContainerFocused,
                          values.datesTarget.length > 0 && {opacity: 0.6},
                        ]}>
                        <TextInput
                          style={[styles.input, {fontFamily: FontFamily.semiBold, fontSize: 18}]}
                          placeholder="e.g. 10"
                  ...
                  <View style={styles.section}>
                    ...
                    <View style={styles.startDateBadge}>
                      <View style={styles.startDateTagContainer}>
                        <AppText style={styles.startDateText}>{values.startDate}</AppText>
                      </View>
                    </View>
                    <AppText style={[styles.switchSublabel, {marginLeft: 0, marginTop: 12}]}>
                      This habit will repeat on the{' '}
                      {getOrdinal(values.startDate ? new Date(values.startDate).getDate() : 0)}
                      {values.frequency === 'yearly'
                        ? ` of ${values.startDate ? new Date(values.startDate).toLocaleString('default', {month: 'long'}) : ''} every year`
                ...
                {!values.isOneTime && values.frequency === 'custom' && (
                  <View style={styles.section}>
                    <View style={styles.sectionHeader}>
                      <AppText style={[styles.sectionTitle, {marginBottom: 0}]}>
                        Specific Dates{' '}
                        {values.specificDatesTarget.length > 0 && `(${values.specificDatesTarget.length})`}
                      </AppText>
                ...
                <View style={styles.section}>
                  ...
                  {touched.goal && typeof errors.goal === 'string' && (
                    <AppText style={styles.errorText}>{errors.goal}</AppText>
                  )}
                    ...
                    {(['hours', 'minutes', 'count', 'none'] as HabitDurationType[]).map(t => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          <Icon name={getIcon()} size={18} color={isSelected ? colors.primary : colors.textSecondary} />
                          <AppText
                            style={[
                              styles.frequencyText,
                              {
                  ...
                  {values.durationType !== 'none' && (
                    <>
                      ...
                      {touched.duration && typeof errors.duration === 'string' && (
                        <AppText style={styles.errorText}>{errors.duration}</AppText>
                      )}
                ...
                <View style={[styles.footer, {borderTopWidth: 0, marginTop: 24, paddingHorizontal: 0}]}>
                  {__DEV__ && !isValid && (
                    <View style={styles.debugContainer}>
                      <AppText style={styles.debugErrorText}>Required: {Object.keys(errors).join(', ')}</AppText>
                    </View>
                  )}
